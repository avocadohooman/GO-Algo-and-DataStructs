...
func main() {
	...
	fmt.Println("Result: ", TwoNumberSum(array, target));
}
...
func TwoNumberSum(array []int, target int) []int {
	...
	targetOne := 0;
	for i := 0; i < len(array) && targetOne < len(array); i++ {
		if (array[targetOne] + array[i] == target && array[targetOne] != array[i]) {
			result = append(result, array[targetOne]);
